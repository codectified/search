...
W(f"| **ES search (ms)** | The ES kNN call: HNSW graph traversal with `num_candidates={FETCH * 5}`, `isChainRef` filter applied inside kNN, post-retrieval `dupGroup` dedup, and network to/from the ES container. |")
...
W(f"| Candidate pool | `num_candidates={FETCH * 5}`, `k={FETCH}`, then dedup to top {N} |")
