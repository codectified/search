...
lines = [
    ...
    "| Tier | Count | % | Meaning |",
    "|---|---|---|---|",
    "| **clean** | %d | %.1f%% | Meaningful isnad prefix stripped (>20 chars, recognizable isnad language) |" % (tiers["clean"], tiers["clean"]/total*100),
    "| **minor** | %d | %.1f%% | Only whitespace/HTML cleanup or trivially short strip |" % (tiers["minor"], tiers["minor"]/total*100),
    "| **unchanged** | %d | %.1f%% | hadithText and englishMatn identical after HTML-stripping |" % (tiers["unchanged"], tiers["unchanged"]/total*100),
    ...
    "",
    "**Effective separation rate**: %.1f%% of hadiths had meaningful isnad stripped  " % (tiers["clean"]/total*100),
    "**Dirty remainder** (unchanged + minor + over_strip + no_matn): %.1f%%\n" % ((total - tiers["clean"])/total*100),
    "",
