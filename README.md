...
```bash
# 1. Build the semantic index (slow on CPU-only instances — use HF endpoint if available)
http://<server>:7650/index?password=<INDEXING_PASSWORD>&targets=<model-key>

# 2. Compute duplicate groups from the semantic vectors (~2-3 min)
